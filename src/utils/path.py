from os import environ, getlogin, name as os_name
...
class Paths:
    ...
    def get_user_data_dir():
        ...
        if os_name == "nt":
            if (appdata_path := environ.get("APPDATA")) is not None:
                base = Path(appdata_path)
            else:
                username = getlogin()
                base = Path(f"C:/Users/{username}/AppData")
        else:
